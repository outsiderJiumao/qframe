...
func New(d []time.Time) Column {
	return Column{data: d}
}

// func (c Column) Get(i uint32) time.Time {
// 	return c.data[i]
// }

// func (c Column) Set(i uint32, val time.Time) {
// 	c.data[i] = val
// }

func NewConst(val time.Time, count int) Column {
	var nullVal time.Time
...
func (c Column) Apply1(fn interface{}, ix index.Int) (interface{}, error) {
	...
	case func(time.Time) time.Time:
		result := make([]time.Time, len(c.data))
		for _, i := range ix {
			result[i] = t(c.data[i])
		}
		return result, nil
...
func (c Column) Apply2(fn interface{}, s2 column.Column, ix index.Int) (column.Column, error) {
	...
	t, ok := fn.(func(time.Time, time.Time) time.Time)
	if !ok {
...
func (c Column) Aggregate(indices []index.Int, fn interface{}) (column.Column, error) {
	var actualFn func([]time.Time) time.Time
	var ok bool
...
func (c Column) subsetWithBuf(index index.Int, buf *[]time.Time) Column {
	if cap(*buf) < len(index) {
		*buf = make([]time.Time, 0, len(index))
	}
...
func (v View) ItemAt(i int) time.Time {
